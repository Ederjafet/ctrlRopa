...
REPORT_MD="$REPORT_DIR/AUTH-J2-security-audit-summary-smoke-report-$REPORT_TS.md"
REPORT_CSV="$REPORT_DIR/AUTH-J2-security-audit-summary-smoke-report-$REPORT_TS.csv"
...
write_report_headers() {
  {
    printf '# AUTH-J2 security audit summary smoke report\n\n'
...
assert_json_field() {
  ...
  if grep -q "\"$field\"" "$file"; then
    record_result "$label" "GET /api/security/audit-events/summary" "SUPPORT" "campo $field" "FOUND" "PASS" "Campo presente"
    pass "$label found $field"
  else
    record_result "$label" "GET /api/security/audit-events/summary" "SUPPORT" "campo $field" "MISSING" "FAIL" "Campo ausente"
...
if [ -n "${SUPPORT_TOKEN:-}" ]; then
  SUMMARY_OUTPUT="$TMP_DIR/summary.json"
  SUMMARY_STATUS="$(request_with_token "GET" "/api/security/audit-events/summary" "$SUPPORT_TOKEN" "$SUMMARY_OUTPUT")"
  expect_status "soporte consulta summary" "GET /api/security/audit-events/summary" "SUPPORT" "$SUMMARY_STATUS" "200" "Debe consultar resumen"

  if [ "$SUMMARY_STATUS" = "200" ]; then
    assert_json_field "summary totalEvents" "$SUMMARY_OUTPUT" "totalEvents"
    assert_json_field "summary byEventType" "$SUMMARY_OUTPUT" "byEventType"
    assert_json_field "summary byStatusCode" "$SUMMARY_OUTPUT" "byStatusCode"
    assert_json_field "summary topPaths" "$SUMMARY_OUTPUT" "topPaths"
  fi
else
  record_result "soporte consulta summary" "GET /api/security/audit-events/summary" "SUPPORT" "200" "SKIP" "SKIP" "No hubo token soporte"
...
if [ -n "${ADMIN_TOKEN:-}" ]; then
  ADMIN_SUMMARY_OUTPUT="$TMP_DIR/admin-summary.json"
  ADMIN_SUMMARY_STATUS="$(request_with_token "GET" "/api/security/audit-events/summary" "$ADMIN_TOKEN" "$ADMIN_SUMMARY_OUTPUT")"
  expect_status "admin tenant sin VIEW_SECURITY_AUDIT" "GET /api/security/audit-events/summary" "TENANT_ADMIN" "$ADMIN_SUMMARY_STATUS" "403" "Debe quedar bloqueado"
else
  record_result "admin tenant sin VIEW_SECURITY_AUDIT" "GET /api/security/audit-events/summary" "TENANT_ADMIN" "403" "SKIP" "SKIP" "No hubo token admin"
  SKIP_COUNT=$((SKIP_COUNT + 1))
fi
