...
| LIVE-Z10B-001 | Arquitectura / producto | Docs | Revisar diseno especifico de autorizacion de precio LIVE | Leer `docs/LIVE_Z10B_PRICE_AUTHORIZATION_DESIGN.md`; validar reglas, permisos propuestos, modelo, endpoints, eventos, UX y plan Z10C/Z10D | Arquitectura emite GO/NO-GO antes de implementar backend, frontend, permisos reales o migraciones de autorizacion de precio | Documento LIVE-Z10B | PENDING_ARCH_REVIEW | Pendiente | Pendiente | S1 |
...
| UX-B2-001 | `qa.admin@local.test` | `/users-form` | Permisos directos legibles por idioma | En espanol abrir `Permisos directos adicionales`; revisar lista; abrir detalles tecnicos; cambiar a ingles y repetir | En espanol no mezcla ingles; codigos internos ocultos por defecto; detalles tecnicos muestran codigo; ingles muestra etiquetas principales en ingles; RBAC no cambia | Usuario admin y catalogo de permisos existente | PENDING_QA | Pendiente | Pendiente | S2 |
