...
| Consulta de auditoria sin permiso dedicado | MEDIO | MEDIA despues de AUTH-H | `MANAGE_SECURITY_SETTINGS` permite consultar auditoria; si el negocio quiere separar lectura de configuracion, el permiso es demasiado amplio. | AUTH-H usa permiso existente para no crear RBAC nuevo; evaluar `VIEW_SECURITY_AUDIT` en fase futura. | Mantener consulta solo para soporte/admin seguridad o retirar endpoint hasta aprobar permiso dedicado. |
