...
| 4 | Rol `SELLER` o `QA_TENANT_SELLER` con canal `LIVE` y `DO_LIVE_RESERVATION` | `SELLER_OPERATION_VIEW` | Flujo de apartado limitado por permisos reales. |
| 5 | Rol `SELLER` o `QA_TENANT_SELLER` sin permiso operativo LIVE | `SELLER_PRESENTER_VIEW` | Vista de apoyo centrada en prenda al aire. |
| 6 | Cualquier caso no cubierto de forma segura | `NO_ACCESS_VIEW` | Bloqueo conservador. |
...
| `SELLER_OPERATION_VIEW` | Estado LIVE, prenda al aire, precio, cliente/interesado y apartados recientes. | Apartar en LIVE si tiene `DO_LIVE_RESERVATION`; seleccionar cliente con `VIEW_CUSTOMERS`; crear cliente con `CREATE_CUSTOMER`; cerrar como venta LIVE solo si tiene capacidad de venta operativa. | Iniciar/cerrar LIVE, cambiar prenda al aire, cancelar sin `CANCEL_RESERVATION`, dashboard supervisor, pagos/caja/reportes. |
| `SELLER_PRESENTER_VIEW` | Estado LIVE, prenda al aire, precio, codigo, talla, color, stock, notas basicas. | Actualizacion ligera por polling; no opera apartados si no tiene permiso operativo. | Dashboard supervisor, consola operador, pagos/caja/reportes. |
...
| `qa.vendedor.centro@local.test` | `SELLER` | `DEFAULT / QA_CTR` (`companyId=1`, `branchId=4`) | `DO_LIVE_RESERVATION`, `VIEW_CUSTOMERS`, `VIEW_INVENTORY`, `REGISTER_PAYMENTS`, `DO_DOOR_SALE`, `DO_DOOR_RESERVATION`. | `SELLER_OPERATION_VIEW` si canal LIVE esta activo | Ver prenda al aire, precio y apartados recientes; seleccionar cliente; crear apartado LIVE; cerrar como venta LIVE si conserva `DO_DOOR_SALE`. | Iniciar/cerrar LIVE, preparar/cambiar prenda al aire, cancelar sin `CANCEL_RESERVATION`, dashboard supervisor, pagos/caja/reportes. | Z5/Z6 lo dejaban en apoyo aunque tenia permiso operativo. | LIVE-Z9G usa permisos reales para darle flujo de apartado sin escalarlo a admin. |
...
LIVE-Z9G corrige la regla de vendedor: `SELLER` con canal `LIVE` + `DO_LIVE_RESERVATION` ya no queda forzado a apoyo visual. Entra al flujo de apartado y cada accion queda condicionada por capacidad real. `SELLER` sin permiso operativo permanece en vista de apoyo.
