...
const PERMISSION_LABELS_ES: Record<string, string> = {
  ...
  EXECUTE_REFUND: 'Ejecutar devolucion',
  ...
  ADMIN_SHIPMENTS: 'Administrar envios',
  ...
  return getMissingSuggestedDependencies(permissionCode, selectedPermissionCodes).map((dependencyCode) => {
    ...
    return dependency
      ...
      : {
          ...
          text: isSpanish
            ? `Dependencia pendiente de definir en catalogo: ${formatPermissionCode(dependencyCode, language)}`
